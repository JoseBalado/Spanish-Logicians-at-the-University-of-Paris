...
| Complete family | General form of $A:B$ | Greater example | Reciprocal example |
| --- | --- | --- | --- |
| equality | $A:A$ | $4:4$ | unchanged |
| _multiplex_ | $m:1$ | _quadrupla_ $4:1$ | _subquadrupla_ $1:4$ |
| _superparticularis_ | $(n+1):n$ | _sesquiquarta_ $5:4$ | _subsesquiquarta_ $4:5$ |
| _superpartiens_ | $(n+k):n$, $1<k<n$, $\gcd(k,n)=1$ | _superbipartiens quintas_ $7:5$ | _subsuperbipartiens quintas_ $5:7$ |
| _multiplex superparticularis_ | $(mn+1):n$ | _tripla sesquitertia_ $10:3$ | _subtripla sesquitertia_ $3:10$ |
| _multiplex superpartiens_ | $(mn+k):n$, $1<k<n$, $\gcd(k,n)=1$ | _tripla superbipartiens quintas_ $17:5$ | _subtripla superbipartiens quintas_ $5:17$ |

The restriction $\gcd(k,n)=1$ expresses the classification after reduction. If $k/n$ reduces to $1/d$, the ratio is superparticular rather than superpartient. These six rows therefore cover equality and all five genera in each direction; the named species are generated by changing $m$, $k$, and $n$.
...
### Proportion and proportionality are different levels

Dolz now passes from a relation between quantities to a relation between proportions:

$$
\underbrace{A:B}_{\text{proportion}}
\qquad\longrightarrow\qquad
\underbrace{(A:B):(C:D)}_{\text{proportionality}}.
$$
...
each descending comparison of a later term to the preceding one is double,
...
| --- | --- | --- | --- |
| double / subdouble | $1,2,4,8$ | $8,4,2,1$ | $1,2,4$ |
| triple / subtriple | $1,3,9,27$ | $27,9,3,1$ | $2,6,18$ |
| quadruple / subquadruple | $1,4,16,64$ | $64,16,4,1$ | $3,12,48$ |
